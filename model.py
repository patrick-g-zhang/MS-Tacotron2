...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        self.dense_spk_embeds = nn.Linear(
            hparams.spks_embedding_dim, hparams.encoder_embedding_dim)
        self.dense_init_lstm = nn.Linear(
            hparams.spks_embedding_dim, 2 * hparams.encoder_embedding_dim)
    ...
    def forward(self, x, input_lengths, spk_embeds):
        ...
        encoder_init_state = encoder_init_state.view(
            -1, encoder_init_state.size(1) // 4, 2, 2)  # (B, 256, 2, 2)
        encoder_init_state = encoder_init_state.permute(
            3, 2, 0, 1)  # (2,2, B, 256)
        outputs, _ = self.lstm(
            x, (encoder_init_state[0].contiguous(), encoder_init_state[1].contiguous()))

        outputs, _ = nn.utils.rnn.pad_packed_sequence(
            outputs, batch_first=True)
        after_lstm = F.softsign(self.dense_spk_embeds(
            spk_embeds))  # (B, 16) -> (B,512)
        after_lstm = after_lstm.unsqueeze(1)  # (B, 512) -> (B, 1, 512)
        after_lstm = after_lstm.repeat(
            1, outputs.size(1), 1)  # (B, max_len, 512)
        # add after conv input and spk_embeds
        outputs = after_lstm + outputs  # (B, max_len, 512)
        return outputs

    def inference(self, x, spk_embeds):
        # only one sample each inference process
        # pdb.set_trace()
        for conv in self.convolutions:
            x = F.dropout(F.relu(conv(x)), 0.5, self.training)
        ...
        # pass spk_embeds to dense net and expand spk_embeds to 3 dims
        # before_lstm = F.softsign(self.dense_spk_embeds(
        #     spk_embeds))  # (1, 16) -> (1,512)
        # before_lstm = before_lstm.unsqueeze(1)  # (1, 512) -> (1, 1, 512)
        # before_lstm = before_lstm.repeat(
        #     1, x.size(1), 1)  # (1, len, 512)

        # # add after conv input and spk_embeds
        # x = before_lstm + x  # (1, len, 512)

        self.lstm.flatten_parameters()

        encoder_init_state = F.softsign(
            self.dense_init_lstm(spk_embeds))  # (1, 512*2)
        encoder_init_state = encoder_init_state.view(
            -1, encoder_init_state.size(1) // 4, 2, 2)  # (1, 256, 2, 2)
        encoder_init_state = encoder_init_state.permute(
            3, 2, 0, 1)  # (2,2, 1, 256)
...
class Decoder(nn.Module):
    ...
    def forward(self, memory, spk_embeds, decoder_inputs, memory_lengths):
        ...
        self.initialize_decoder_states(
            memory, spk_embeds, mask=~get_mask_from_lengths(memory_lengths))
    ...
    def inference(self, memory, spk_embeds):
        """ Decoder inference
        PARAMS
        ------
        memory: Encoder outputs
        ...
        self.initialize_decoder_states(memory, spk_embeds, mask=None)
...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        self.spk_embedding = nn.Embedding(
            hparams.n_spks, hparams.spks_embedding_dim)
        std = sqrt(2.0 / (hparams.n_spks + hparams.spks_embedding_dim))
        val = sqrt(3.0) * std  # uniform bounds for std
        self.spk_embedding.weight.data.uniform_(-val, val)
    ...
    def parse_batch(self, batch):
        ...
        return (
            (text_padded, input_lengths, mel_padded,
             max_len, output_lengths, spk_ids),
    ...
    def forward(self, inputs):
        text_inputs, text_lengths, mels, max_len, output_lengths, spk_ids = inputs
        text_lengths, output_lengths = text_lengths.data, output_lengths.data

        embedded_inputs = self.embedding(text_inputs).transpose(1, 2)
        spk_embeds = self.spk_embedding(spk_ids)  # (B, 16)

        # pdb.set_trace()
        encoder_outputs = self.encoder(
            embedded_inputs, text_lengths, spk_embeds)  # (B, max_len, 512)

        mel_outputs, gate_outputs, alignments = self.decoder(
            encoder_outputs, spk_embeds, mels, memory_lengths=text_lengths)
    ...
    def inference(self, inputs, spk_ids):
        embedded_inputs = self.embedding(inputs).transpose(1, 2)

        spk_embeds = self.spk_embedding(spk_ids)  # (B, 16)

        encoder_outputs = self.encoder.inference(embedded_inputs, spk_embeds)
        mel_outputs, gate_outputs, alignments = self.decoder.inference(
            encoder_outputs, spk_embeds)
